...
ReadRPNCode* FindExistingRPNCode(const std::string code) {
...
void RegisterFloatSimVar(const std::string code, Client* client) {
	...
	SimVar newSimVar;
	ReadRPNCode* pdupRpn = FindExistingRPNCode(code);
	HRESULT hr;
...
void RegisterStringSimVar(const std::string code, Client* client) {
	...
	if (pdupRpn) {
		...
		ReadRPNCode rpnCode;
		rpnCode.Code = code;
		rpnCode.RetType = RPN_TYPE_STRING;
